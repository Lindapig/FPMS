...
BEGIN
    ...
    -- Calculate the average cost basis per share using FIFO
...
BEGIN
    ...
    INSERT INTO transaction(account_id, transaction_date, transaction_type, transaction_amount, security_id)
    VALUES ((SELECT account_id FROM account WHERE user_id = p_user_id), CURDATE(), 'Deposit', p_amount, NULL);
END$$
...
BEGIN
    IF (SELECT account_balance FROM account WHERE user_id = p_user_id) >= p_amount THEN
        ...
        SET account_balance = account_balance - p_amount WHERE user_id = p_user_id;
        INSERT INTO transaction(account_id, transaction_date, transaction_type, transaction_amount, security_id)
        VALUES ((SELECT account_id FROM account WHERE user_id = p_user_id), CURDATE(), 'Withdraw', p_amount, NULL);
    ELSE
