...
BEGIN
    ...
    IF v_total_cost <= v_account_balance THEN
        ...
        INSERT INTO transaction(account_id, transaction_date, transaction_type, transaction_amount, security_id, quantity)
        VALUES (v_account_id, CURDATE(), 'Buy', v_total_cost, p_security_id, p_quantity);
    ELSE
...
DELIMITER ;
-- test
CALL check_withholding_security(1, 1);
...
BEGIN
    ...
    DECLARE v_account_id INT;
    DECLARE v_cost_base DECIMAL(10, 2); -- Variable to store the cost base
    ...
    -- Retrieve the latest buy transaction for the security in the portfolio
    SELECT t.transaction_amount / t.quantity INTO v_cost_base
    FROM transaction t
    JOIN security_in_portfolio sp ON t.security_id = sp.security_id
    WHERE t.transaction_type = 'Buy' 
        AND sp.portfolio_id = p_portfolio_id 
        AND sp.security_id = p_security_id
    ...
    IF v_quantity >= p_quantity THEN
        ...
        SET 
            ...
            gain_loss = (quantity - p_quantity) * v_price - (p_quantity * v_cost_base) -- Update the gain/loss
        WHERE 
            security_id = p_security_id AND portfolio_id = p_portfolio_id;
